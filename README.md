...
- **App Router 기반 SSR + Hydration 설계**: 대시보드 진입 시 서버에서 핵심 데이터를 선패치하고 클라이언트에서 즉시 상호작용 가능한 상태로 이어집니다.
...
### 1) SSR 선패치 + 클라이언트 하이드레이션

대시보드 상세 페이지에서 서버 단계에 `dashboard`, `columns`, `cards`를 선패치하고 `dehydrate` 상태를 전달합니다.  
첫 페인트 시점부터 데이터가 준비되어 있고, 이후 클라이언트 상호작용은 React Query가 이어받아 처리합니다.
